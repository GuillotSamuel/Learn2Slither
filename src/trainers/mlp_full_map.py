...
class MLP(nn.Module):
    ...
    It takes input vectors representing the full game board state
    (board_size² + 4 direction features) and outputs vectors of
    size 3 representing Q-values for actions.
    ...
    Typical usage:
        model = MLP(input_size=104)  # For 10x10 board
        output = model.forward(input_data)

    This architecture is suitable for Deep Q-Learning with
    ...
    def __init__(self, input_size=104):
        ...
        The network takes input vectors representing the full
        board state and outputs Q-values for 3 possible actions.

        Args:
            input_size (int): Size of the input vector 
            (board_size² + 4 for direction). Default is 104 for 10x10 board.
        ...
        Example:
            >>> model = MLP(input_size=104)  # For 10x10 board
            >>> model = MLP(input_size=29)   # For 5x5 board
        """
